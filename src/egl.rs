...
pub fn choose_config(
    ...
    unsafe {
        ...
        if ffi::eglChooseConfig(display, attribs, &mut config, config_size, &mut count) == EGL_TRUE
        {
            if count > 0 {
                Some(config)
            } else {
...
pub fn copy_buffers(display: EGLDisplay, surface: EGLSurface, target: EGLNativePixmapType) -> bool {
    unsafe { ffi::eglCopyBuffers(display, surface, target) == EGL_TRUE }
}
...
pub fn destroy_context(display: EGLDisplay, ctx: EGLContext) -> bool {
    unsafe { ffi::eglDestroyContext(display, ctx) == EGL_TRUE }
}

pub fn destroy_surface(display: EGLDisplay, surface: EGLSurface) -> bool {
    unsafe { ffi::eglDestroySurface(display, surface) == EGL_TRUE }
}
...
pub fn get_config_attrib(
    ...
    unsafe { ffi::eglGetConfigAttrib(display, config, attribute, value) == EGL_TRUE }
}
...
pub fn initialize(display: EGLDisplay, major: &mut EGLint, minor: &mut EGLint) -> bool {
    unsafe { ffi::eglInitialize(display, major, minor) == EGL_TRUE }
}
...
pub fn make_current(
    ...
    unsafe { ffi::eglMakeCurrent(display, draw, read, ctx) == EGL_TRUE }
}
...
pub fn query_context(
    ...
    unsafe { ffi::eglQueryContext(display, ctx, attribute, value) == EGL_TRUE }
}
...
pub fn query_surface(
    ...
    unsafe { ffi::eglQuerySurface(display, surface, attribute, value) == EGL_TRUE }
}

pub fn swap_buffers(display: EGLDisplay, surface: EGLSurface) -> bool {
    unsafe { ffi::eglSwapBuffers(display, surface) == EGL_TRUE }
}

pub fn terminate(display: EGLDisplay) -> bool {
    unsafe { ffi::eglTerminate(display) == EGL_TRUE }
}

pub fn wait_gl() -> bool {
    unsafe { ffi::eglWaitGL() == EGL_TRUE }
}

pub fn wait_native(engine: EGLint) -> bool {
    unsafe { ffi::eglWaitNative(engine) == EGL_TRUE }
}
...
pub fn bind_tex_image(display: EGLDisplay, surface: EGLSurface, buffer: EGLint) -> bool {
    unsafe { ffi::eglBindTexImage(display, surface, buffer) == EGL_TRUE }
}

pub fn release_tex_image(display: EGLDisplay, surface: EGLSurface, buffer: EGLint) -> bool {
    unsafe { ffi::eglReleaseTexImage(display, surface, buffer) == EGL_TRUE }
}
...
pub fn surface_attrib(
    ...
    unsafe { ffi::eglSurfaceAttrib(display, surface, attribute, value) == EGL_TRUE }
}

pub fn swap_interval(display: EGLDisplay, interval: EGLint) -> bool {
    unsafe { ffi::eglSwapInterval(display, interval) == EGL_TRUE }
}
...
pub fn bind_api(api: EGLenum) -> bool {
    unsafe { ffi::eglBindAPI(api) == EGL_TRUE }
}
...
pub fn create_pbuffer_from_client_buffer(
    ...
    unsafe {
        ...
        let surface =
            ffi::eglCreatePbufferFromClientBuffer(display, buffer_type, buffer, config, attribs);
...
pub fn release_thread() -> bool {
    unsafe { ffi::eglReleaseThread() == EGL_TRUE }
}

pub fn wait_client() -> bool {
    unsafe { ffi::eglWaitClient() == EGL_TRUE }
}
...
pub fn destroy_sync(dpy: EGLDisplay, sync: EGLSync) -> bool {
    unsafe { ffi::eglDestroySync(dpy, sync) == EGL_TRUE }
}

pub fn client_wait_sync(dpy: EGLDisplay, sync: EGLSync, flags: EGLint, timeout: EGLTime) -> EGLint {
    unsafe { ffi::eglClientWaitSync(dpy, sync, flags, timeout) }
}
...
pub fn destroy_image(dpy: EGLDisplay, image: EGLImage) -> bool {
    unsafe { ffi::eglDestroyImage(dpy, image) == EGL_TRUE }
}
...
pub fn wait_sync(dpy: EGLDisplay, sync: EGLSync, flags: EGLint) -> bool {
    unsafe { ffi::eglWaitSync(dpy, sync, flags) == EGL_TRUE }
}
...
mod ffi {
    use super::{
        EGLAttrib, EGLBoolean, EGLClientBuffer, EGLConfig, EGLContext, EGLDisplay, EGLImage,
        EGLNativeDisplayType, EGLNativePixmapType, EGLNativeWindowType, EGLSurface, EGLSync,
        EGLTime, EGLenum, EGLint,
    };
